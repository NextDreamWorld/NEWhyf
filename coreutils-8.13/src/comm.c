...
static char const *delimiter;    //  ‰≥ˆ∑÷∏Ó∑˚£¨ƒ¨»œŒ™tab

/* For long options that have no equivalent short option, use a
   non-character as a pseudo short option, starting with CHAR_MAX + 1.  */
enum
{
  CHECK_ORDER_OPTION = CHAR_MAX + 1,  // CHAR_MAX”¶∏√ «127?  ”–¥˝—È÷§
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      thisline[i] = readlinebuffer (all_line[i][alt[i][0]], streams[i]);// readlinebuffer ’‚∏ˆ∫Ø ˝¿¥◊‘ƒƒ¿?
      if (ferror (streams[i]))
      
        error (EXIT_FAILURE, errno, "%s", infiles[i]);
    }

  while (thisline[0] || thisline[1])   // œ¬“ª––≤ªŒ™ø’
    {
      int order;                       // ≈–∂œ¡Ω––µƒΩ·π˚£¨1:µ⁄“ª∏ˆŒƒº˛¥Û -1:µ⁄∂˛∏ˆŒƒº˛¥Û 0:“ª—˘¥Û
      bool fill_up[2] = { false, false };

      /* Compare the next available lines of the two files.  */

      if (!thisline[0])                // µ⁄“ª∏ˆŒƒº˛œ¬“ª––Œ™ø’
        order = 1;
      else if (!thisline[1])		   // µ⁄∂˛∏ˆŒƒº˛œ¬“ª––Œ™ø’
        order = -1;
      else                            // œ¬“ª––∂º≤ªŒ™ø’
        {
          if (hard_LC_COLLATE)    //collate  –£∂‘
      ...
      if (order == 0)
        writeline (thisline[1], stdout, 3);    //¡Ω––œ‡µ»
      else
        {
      ...
      if (0 <= order)
        fill_up[1] = true;    // Œƒº˛2 »°œ¬“ª––
      if (order <= 0)
        fill_up[0] = true;    //Œƒº˛1 »°œ¬“ª––
...
main (int argc, char **argv)
{
  ...
  only_file_1 = true;   // œ‘ æ÷ª”–µ⁄“ª∏ˆŒƒº˛÷–”–ƒ⁄»›
  only_file_2 = true;	  // œ‘ æ÷ª”–µ⁄∂˛ ∏ˆŒƒº˛÷–”–ƒ⁄»›
  both = true;          //  œ‘ æ¡Ω∏ˆŒƒº˛π≤”–µƒƒ⁄»›

  seen_unpairable = false;
  issued_disorder_warning[0] = issued_disorder_warning[1] = false;
  check_input_order = CHECK_ORDER_DEFAULT;

  //  ?? getopt_long()Ω‚Œˆ√¸¡Ó––—°œÓ≤Œ ˝
  while ((c = getopt_long (argc, argv, "123", long_options, NULL)) != -1)
    switch (c)
      {
      case '1':
        only_file_1 = false;  // -1 ≤ªœ‘ æ÷ª”–Œƒº˛1÷–”–µƒƒ⁄»›
        break;

      case '2':
        only_file_2 = false; // -2 ≤ªœ‘ æ÷ª”–Œƒº˛2÷–”–µƒƒ⁄»›
        break;

      case '3':				   // -3 ≤ªœ‘ æ¡Ω∏ˆŒƒº˛π≤”–µƒƒ⁄»›
        both = false;
        break;

      case NOCHECK_ORDER_OPTION: // ≤ªºÏ≤È√¸¡Ó «∑Ò’Ê»∑
        check_input_order = CHECK_ORDER_DISABLED;
        break;

      case CHECK_ORDER_OPTION:  // ºÏ≤È√¸¡Ó «∑Ò’Ê»∑
        check_input_order = CHECK_ORDER_ENABLED;
        break;

      case OUTPUT_DELIMITER_OPTION: // ÷∏∂®∑÷∏Ù∑˚
        if (delimiter && !STREQ (delimiter, optarg))
          error (EXIT_FAILURE, 0, _("multiple delimiters specified"));    // ’‚¿Ôµƒ¥ÌŒÛ–≈œ¢ª·≤ªª·ÕÀ≥ˆ≥Ã–Ú??
        delimiter = optarg;
        if (!*delimiter)
          {
      ...
      case_GETOPT_HELP_CHAR;     //  œ‘ æ∞Ô÷˙

      case_GETOPT_VERSION_CHAR (PROGRAM_NAME, AUTHORS); //œ‘ æ∞Ê±æ–≈œ¢∫Õ◊˜’ﬂ
